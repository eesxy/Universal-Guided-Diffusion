...
class DDIMSamplerWithGrad(object):
    ...
    def sample(self, S, batch_size, shape, operated_image=None, operation=None, conditioning=None,
               eta=0., temperature=1., verbose=True, unconditional_guidance_scale=1.,
               unconditional_conditioning=None, start_zt=None):
        ...
        for i, step in enumerate(iterator):
            ...
            for j in range(num_steps):
                ...
                if operation.guidance_3:
                    ...
                    if op_im is not None:
                        if hasattr(operation_func, 'cal_loss'):
                            selected = -1 * operation_func.cal_loss(recons_image,
                                                                    operated_image).unsqueeze(0)
                ...
                with torch.no_grad():
                    ...
                    img = beta_t.sqrt() * x_prev + (1 - beta_t).sqrt() * noise_like(
                        img.shape, device, False)
            ...
            for j in range(num_steps):
                ...
                if operation.guidance_3:
                    ...
                    if operation.original_guidance:
                        ...
                        t_in = torch.cat([ts] * 2)
                        c_in = torch.cat([unconditional_conditioning, cond])
                        e_t_uncond, e_t_cond = self.model.module.apply_model(x_in, t_in,
                                                                             c_in).chunk(2)
                        e_t = e_t_uncond  # + unconditional_guidance_scale * (e_t - e_t_uncond)
                        # del x_in
                    else:
                    ...
                    if op_im is not None:
                        ...
                        e_t = e_t - sqrt_one_minus_at * grad.detach(
                        ) + unconditional_guidance_scale * (e_t_cond - e_t_uncond)
    ...
    def sample_combine(
        ...
        for i, step in enumerate(iterator):
            ...
            num_steps = operation_od.num_steps[0]
            operation_func_od = operation_od.operation_func
            operation_func_fd = operation_fd.operation_func
            criterion_od = operation_od.loss_func
            criterion_fd = operation_fd.loss_func
            other_criterion_od = operation_od.other_criterion
            other_criterion_fd = operation_fd.other_criterion

            for j in range(num_steps):
                if operation_od.guidance_3:

                    torch.set_grad_enabled(True)
                    img_in = img.detach().requires_grad_(True)

                    if operation_od.original_guidance:
                        x_in = torch.cat([img_in] * 2)
                        t_in = torch.cat([ts] * 2)
                        c_in = torch.cat([unconditional_conditioning, cond])
                        e_t_uncond, e_t_cond = self.model.module.apply_model(x_in, t_in,
                    ...
                    op_im_od = operation_func_od(recons_image)
                    op_im_fd = operation_func_fd(recons_image)

                    if op_im_od is not None and op_im_fd is not None:
                        if hasattr(operation_func_od, 'cal_loss'):
                            selected_od = -1 * operation_func_od.cal_loss(
                                recons_image, operated_image_od).unsqueeze(0)
                        elif other_criterion_od != None:
                            selected_od = -1 * other_criterion_od(op_im_od, operated_image_od)
                        else:
                            selected_od = -1 * criterion_od(op_im_od, operated_image_od)

                        if hasattr(operation_func_od, 'cal_loss'):
                            selected_fd = -1 * operation_func_od.cal_loss(
                                recons_image, operated_image_od).unsqueeze(0)
                        elif other_criterion_od != None:
                            selected_fd = -1 * other_criterion_od(op_im_od, operated_image_od)
                        else:
                            selected_fd = -1 * criterion_od(op_im_od, operated_image_od)

                        # print(ts)
                        # print(selected)

                        grad = torch.autograd.grad(
                            selected_od.sum() * operation_od.optim_guidance_3_wt +
                            selected_fd.sum() * operation_fd.optim_guidance_3_wt, img_in)[0]

                        e_t = e_t - sqrt_one_minus_at * grad.detach()

                        del grad, selected_od, selected_fd

                    elif op_im_od is not None:
                        if hasattr(operation_func_od, 'cal_loss'):
                            selected = -1 * operation_func_od.cal_loss(
                                recons_image, operated_image_od).unsqueeze(0)
                        elif other_criterion_od != None:
                            selected = -1 * other_criterion_od(op_im_od, operated_image_od)
                        else:
                            selected = -1 * criterion_od(op_im_od, operated_image_od)

                        # print(ts)
                        # print(selected)

                        grad_od = torch.autograd.grad(
                            selected.sum() * operation_od.optim_guidance_3_wt, img_in)[0]

                        e_t = e_t - sqrt_one_minus_at * grad_od.detach()

                        del grad_od, selected

                    elif op_im_fd is not None:
                        if hasattr(operation_func_fd, 'cal_loss'):
                            selected = -1 * operation_func_fd.cal_loss(
                                recons_image, operated_image_fd).unsqueeze(0)
                        elif other_criterion_fd != None:
                            selected = -1 * other_criterion_fd(op_im_fd, operated_image_fd)
                        else:
                            selected = -1 * criterion_fd(op_im_fd, operated_image_fd)

                        # print(ts)
                        # print(selected)

                        grad_fd = torch.autograd.grad(
                            selected.sum() * operation_fd.optim_guidance_3_wt, img_in)[0]

                        e_t = e_t - sqrt_one_minus_at * grad_fd.detach()

                        del grad_fd, selected
                    ...
                    del img_in, pred_x0, recons_image, op_im_od, op_im_fd, e_t_uncond
                    if operation_od.original_guidance:
                        del x_in
